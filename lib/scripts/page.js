...
exports.run = async (opts = []) => {
  ...
  // 存放页面的目标路径
  const src = opts.dir || woOptions.SRC_PAGES;
  if (!src) {
    //
  ...
  function writeFile(n) {
    ...
    const reName = res[1] || '';
    const contextName = reName.replace(reName.charAt(0), reName.charAt(0).toUpperCase());
    const context = {
      name: contextName,
      rename: noIndexFile ? reName : 'index'
    };
    //
    glob
      .sync('**/*', {
      ...
      .forEach(file => {
        ...
        if (statSync(filePath).isFile()) {
          ...
          if (noIndexFile) {
            //
            newName = [reName, path.extname(file)].join('');
          }
